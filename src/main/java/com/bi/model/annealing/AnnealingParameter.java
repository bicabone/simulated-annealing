...
public enum AnnealingParameter {
  ...
  @SafeVarargs
  AnnealingParameter(Class<? extends SimulatedAnnealingSystem>... classes) {
    this.classes = Arrays.stream(classes).collect(Collectors.toList());
  }

  public static Collection<AnnealingParameter> getRequiredParameters(
      Class<? extends SimulatedAnnealingSystem> clazz) {
    return Arrays.stream(AnnealingParameter.values())
